...
public class Logica_PedidoAJAX implements ILogica_PedidoAJAX {
	...
	public Logica_PedidoAJAX() {
		cliente_ajax = new Clientes_Ajax();
	}

	/**
	 * Consulta de los clientes para la carga de la ventana de ayuda en el
	 * navegador. Patron de precarga
	 * 
	 * @return Objeto de dominio para el movimiento de la informacion.
	 */
	@Override
	public Clientes_Ajax consultar_ClientesAyuda() {
		List<Clientes_Pedido> lista = gestion_clientes.consultar_Todos();
		if(lista.size()==0){
			mensaje="ERROR, NO SE ENCONTRARON CLIENTES PARA LA AYUDA";
		}
		// CARGAMOS LA INFORMACION DEL PROCESO EN EL OBJETO DE DOMINIO
		cliente_ajax.setLista_clientes(lista);
	...
	public void setGestion_clientes(IGestion_Clientes gestion_clientes) {
